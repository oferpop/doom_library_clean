...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not Customer.query.filter_by(mail='oferpop').first():
            hashed_password = bcrypt.generate_password_hash('ok1505').decode('utf-8')
            initial_admin = Customer(name='oferpop', mail='oferpop', city='', age=0, gender='', password=hashed_password, role='admin')
